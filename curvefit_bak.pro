...
;             print,'after ',function_name,': pder = ',pder
            IF (N_ELEMENTS(yfit) NE nY) THEN MESSAGE, error_msg1
...
;          print,'pder[*,iparam] = ',pder[*,iparam]
          tempc = (y-yfit)*Weights
;          print,'tempc = ',tempc
          beta = (nparam eq 1) ? [TOTAL((y-yfit)*Weights*pder[*,iparam])] : $
            (y-yfit)*Weights # pder[*,iparam]
;          print,'beta = ',beta
          tempa = Weights # (FLTARR(nparam)+1)
;          print,'tempa = ',tempa
;          print,'pder = ',pder
          tempa = tempa * pder
;          print,'tempa = ',tempa
          tempb = TRANSPOSE(pder[*,iparam])
;          print,'tempb = ',tempb
          tempc = tempb # tempa
;          print,'tempc = ',tempc
...
;          print,'alpha[diag] = ',alpha[diag]
          sigma1 = sqrt( 1.0 / alpha[diag] )           ; Current sigma.
;          print,'sigma1 = ',sigma1
          sigma[iparam]  = sigma1
;          print,'sigma[iparam] = ',sigma[iparam]
...
;          print,'yfit1 = ',yfit1
          ...
          if done_early then begin
            print,'done_early = true'
            stop
          end else print,'done_early = false'
...
;             print,'array = ',array
;             print,'array # array_temp = ',array # array_temp
;             print,'array ## array_temp = ',array ## array_temp

             b = a
;             print,'array/c = ',array/c
;             print,'array/c # transpose(beta) = ',array/c # transpose(beta)
;             print,'a[iparam] = ',a[iparam]
             b[iparam] = a[iparam] + (array/c) # transpose(beta) ; New params
;             print,'b[iparam] = ',b[iparam]
             IF (nterms EQ 1) THEN b = b[0]             ; Ensure b is a scalar
;             print,'b = ',b

             call_procedure, function_name, x, b, yfit  ; Evaluate function
;             print,'after ',function_name,': yfit = ',yfit
             chisq = total(Weights*(y-yfit)^2)/nfree    ; New chisq
;             print,'chisq = ',chisq
;             print,'array = ',array
;             print,'array[diag=',diag,'] = ',array[diag]
;             print,'alpha = ',alpha
;             print,'alpha[diag] = ',alpha[diag]
             sigma[iparam] = SQRT(array[diag]/alpha[diag])      ; New sigma
;             print,'sigma = ',sigma
;             print,'sigma[iparam=',iparam,'] = ',sigma[iparam]

;             if (lambdaCount eq 2) then begin
;               print,'lambdaCount == 2'
;               stop
;             endif
...
;             print,'lambdaCount = ', lambdaCount, ': flambda = ',flambda
;             stop
