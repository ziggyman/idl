...
pro rave_vrad_errvrad,datafile
;common maxn,dlambda,lambda,flux

;
; NAME:                  rave_vrad_errvrad.pro
; PURPOSE:               plots err_vrad over vrad
; CATEGORY:              rave
; CALLING SEQUENCE:      rave_vrad_errvrad,'/home/azuri/daten/rave/rave data/rave_dr1.dat'
; INPUTS:                datafile
...
;-- test arguments
  if n_elements(datafile) eq 0 then begin
    ...
    print,'Usage: rave_vrad_errvrad,(string)datafile'
    print,"Example: rave_vrad_errvrad,'/home/azuri/daten/rave/rave_data/rave_dr1.dat'"
    datafile = '/home/azuri/daten/rave/rave_data/rave_dr1.dat'
  end
;  print,'rave_vrad_errvrad: datafile = "'+datafile+'"'
;  str_path = strmid(datafile,0,strpos(datafile,'/',/REVERSE_SEARCH)+1)
;  i_ndatalines = countlines(datafile)
;  print,'rave_vrad_errvrad: i_ndatalines = ',i_ndatalines
...
;  device,/close
;  set_plot,'x'
  rave_plot_two_cols,datafile,5,6,'Radial Velocity [km/s]','Error Radial Velocity [km/s]',strmid(datafile,0,strpos(datafile,'.',/REVERSE_SEARCH))+'_vrad_errvrad.ps'
end
