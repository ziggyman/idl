...
pro besancon_write_stars,infile,$
                         outfile,$
...
;-- test arguments
  if n_elements(outfile) eq 0 then begin
    ...
    infile = '/suphys/azuri/daten/besancon/177-183_-36--30/1209132335.045735.resu'
    outfile = '/suphys/azuri/daten/besancon/177-183_-36--30/1209132335.045735.dat'
  end
  ...
  openr,lun,infile,/GET_LUN
    openw,lunw,outfile,/GET_LUN
      for i=0ul,i_ndatalines-1 do begin
        readf,lun,line
        if not(keyword_set(B_EXTINCTION)) then begin
          if strmid(strtrim(line,2),0,4) eq 'Dist' then begin
            i_found_dist = i_found_dist + 1
          end else begin
            if i_found_dist eq 3 then begin
              printf,lunw,line
            endif
          end
        end else begin
          if strmid(strtrim(line,2),0,9) eq 'I     J-K' then begin
            i_found_dist = i_found_dist + 1
            print,'besancon_write_stars: string <I     J-K> found: i_found_dist = ',i_found_dist
          end else begin
