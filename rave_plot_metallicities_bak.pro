...
;  dblarr_chem_snr = double(strarr_data(*,i_col_chem_snr))
  ...
  indarr = where(dblarr_chem_feh gt -9. and dblarr_chem_mh gt -9. and dblarr_rave_mh lt 99.)
  ...
  for i=5, 11 do begin
    ...
    if i eq 0 then begin
      ...
      dblarr_measure_errors(indarr) = 0.25
      dblarr_measure_errors(indarr_40) = 0.175
      dblarr_fit_coeffs_giants_b = svdfit(dblarr_x,dblarr_x-dblarr_y,3,measure_errors = dblarr_measure_errors, yfit=dblarr_yfit)
      dblarr_yfit = dblarr_x - dblarr_yfit
...
;      dblarr_yfit = dblarr_x - dblarr_yfit
      ...
      str_title = 'Chemical [M/H] vs Chemical [M/H] calculated from [Fe/H]'
      besancon_calculate_mh,I_DBLARR_FEH=dblarr_chem_feh,$
                            O_DBLARR_MH=dblarr_chem_mh_calculated ; --- dblarr
...
;      dblarr_fit_coeffs = svdfit(dblarr_x,dblarr_y,4,measure_errors = dblarr_measure_errors, yfit=dblarr_yfit)
      ...
      str_ytitle_b = '([M/H]!DRAVE!N - [M/H]!DChem!N) [dex]'
      str_hist_xtitle = '[M/H]!DRAVE!N and [M/H]!DChem!N [dex]'
      str_x = 'RAVE-MH-DR3sep-STNge13'
      str_y = 'chem-MH'
      dblarr_x = dblarr_rave_mh_calibrated
      dblarr_y = dblarr_chem_mh
    end else if i eq 15 then begin
      str_title = 'RAVE [M/H] (DR3 calibration, separate for dwarfs and giants, STN >= 13) vs Chemical calculated from [Fe/H]'
      str_xtitle = '[M/H]!DRAVE!N [dex]'
      str_ytitle = '[M/H]!DChem!N [dex]'
      str_ytitle_b = '([M/H]!DRAVE!N - [M/H]!DChem!N) [dex]'
      str_hist_xtitle = '[M/H]!DRAVE!N and [M/H]!DChem!N [dex]'
      ...
      str_title = 'giants only: RAVE [M/H] (DR3 calibration, separate for dwarfs and giants, STN >= 13) vs Chemical'
      str_xtitle = '[M/H]!DRAVE, giants!N [dex]'
      str_ytitle = '[M/H]!DChem, giants!N [dex]'
      str_ytitle_b = '([M/H]!DRAVE!N - [M/H]!DChem!N) [dex]'
      str_hist_xtitle = '[M/H]!DRAVE, giants!N and [M/H]!DChem, giants!N [dex]'
      str_x = 'RAVE-MH-DR3sep-STNge13-giants'
      str_y = 'chem-MH'
      indarr_logg_lt_3_5 = where(dblarr_chem_logg lt 3.5,COMPLEMENT=indarr_logg_ge_3_5)
      dblarr_x = dblarr_rave_mh_calibrated(indarr_logg_lt_3_5)
      dblarr_y = dblarr_chem_mh(indarr_logg_lt_3_5)
    end else if i eq 17 then begin
      str_title = 'dwarfs only: RAVE [M/H] (DR3 calibration, separate for dwarfs and giants, STN >= 13) vs Chemical'
      str_xtitle = '[M/H]!DRAVE, dwarfs!N [dex]'
      str_ytitle = '[M/H]!DChem, dwarfs!N [dex]'
      str_ytitle_b = '([M/H]!DRAVE!N - [M/H]!DChem!N) [dex]'
      str_hist_xtitle = '[M/H]!DRAVE, dwarfs!N and [M/H]!DChem, dwarfs!N [dex]'
      str_x = 'RAVE-MH-DR3sep-STNge13-dwarfs'
      str_y = 'chem-MH'
      dblarr_x = dblarr_rave_mh_calibrated(indarr_logg_ge_3_5)
      dblarr_y = dblarr_chem_mh_calculated(indarr_logg_ge_3_5)
    end

    str_plotname_root = strmid(str_plot,0,strpos(str_plot,'.',/REVERSE_SEARCH))+'_'+str_x+'_vs_'+str_y
    ...
    compare_two_parameters,dblarr_x,$
                           ...
                           DBLARR_RAVE_SNR          = dblarr_rave_stn,$
