pro rave_compare_mh_to_feh_with_afe
  ...
  str_indexfile = strmid(str_filename_in,0,strpos(str_filename_in,'/',/REVERSE_SEARCH))+'/feh-from-mh/index.html'
...
;    endelse
  ...
  ;    dblarr_feh(indarr_bad) = 99.9
    ...
    for i=5,25 do begin
      ...
      str_plotname_root = strmid(str_filename_in,0,strpos(str_filename_in,'/',/REVERSE_SEARCH))+'/feh-from-mh/'
      if i eq 5 then begin; --- mine
        str_plotname_root = str_plotname_root + 'FeH_vs_MH'
        str_xtitle = '[M/H]!DS&G!N [dex]'
        str_ytitle = '[Fe/H]!DS&G!N [dex]'
        diff_str_ytitle = '[M/H]!DS&G!N [dex] - [Fe/H]!DS&G!N'
        dblarr_x = dblarr_mh
        dblarr_y = dblarr_feh
        dblarr_yrange_diff = [-0.5,0.5]
      end else if i eq 6 then begin; --- mine
        str_plotname_root = str_plotname_root + 'MH_vs_MH-from-FeH-smoothed-mean'
        str_xtitle = '[M/H]!DS&G!N [dex]'
