pro rave_compare_to_external_and_calibrate, I_STR_PATH         = i_str_path,$
  ...
  b_do_all = 1
  b_with_feh = 1
  b_calib_with_afe_first_run_only = 0
  b_calib_without_afe = 0
  b_do_calib_x_vs_ext = 0
  b_calib_vs_raw_data = 0
  ...
  if b_calib_without_afe then begin
    int_cal_end -= 1
  endif
  ...
  if keyword_set(I_STR_PATH) then begin
    ...
    str_path = '/home/azuri/daten/rave/calibration/MH-from-FeH-and-aFe/'
    spawn,'mkdir '+str_path
    str_path = str_path + '2d/'
    spawn,'mkdir '+str_path
    if b_calib_with_afe_first_run_only then begin
      str_path = str_path + 'aFe_first_run_only/'
      spawn,'mkdir '+str_path
    end else if b_calib_without_afe then begin
      ...
      spawn,'mkdir '+str_path
    endelse
    if b_calib_mh then begin
      str_path = str_path + 'calib_mh/'
    end else if b_calib_cmh then begin
      str_path = str_path + 'calib_cmh/'
    end else if b_calib_mh_cmh then begin
      str_path = str_path + 'calib_mh_cmh/'
    endif
    spawn,'mkdir '+str_path
    if b_calib_vs_raw_data then begin
      str_path = str_path + 'x-raw_'+strtrim(string(int_niter),2)+'iterations/'
    end else begin
      str_path = str_path + 'x-calib_'+strtrim(string(int_niter),2)+'iterations/'
    endelse
    spawn,'mkdir '+str_path
    if b_with_ruchti then begin
      str_path = str_path + 'with-ruchti/'
    endif
  endelse
  spawn,'mkdir '+str_path
  ...
  if keyword_set(I_STR_FILENAME) then begin
    ...
    str_filename = '/home/azuri/daten/rave/calibration/all_found_mh-from-feh-afe'
  endelse
  if b_with_ruchti then $
    str_filename = str_filename + '_with-ruchti'
  str_filename = str_filename + '.dat'
  ...
  if keyword_set(I_STR_RAVEFILENAME) then begin
    ...
    str_filename_rave = '/home/azuri/daten/rave/rave_data/release8/rave_internal_dr8_all_with-2MASS-JK_minus-ic1-ic2_230-315_-25-25_JmK2MASS_gt_0_5_no-flag_STN-gt-20-with-atm-par_no-doubles-within-2-arcsec-maxsnr.dat';rave_internal_dr8_all_with-2MASS-JK_minus-ic1_230-315_-25-25_JmK2MASS_gt_0_5_no_doubles_maxsnr_I2MASS_9ltIlt12_good_STN-gt-20-with-atm-par.dat'
  endelse
;  print,'str_filename_rave = ',str_filename_rave
;  stop
  str_filename_rave_out = str_path + strmid(str_filename_rave,strpos(str_filename_rave,'/',/REVERSE_SEARCH)+1)
  str_filename_rave_out = strmid(str_filename_rave_out,0,strpos(str_filename_rave_out,'.',/REVERSE_SEARCH))+'_calib-STN-Teff-mH-logg-aFe_MH-from-FeH-and-aFe_'+strtrim(string(int_niter),2)+'iterations.dat'
  ...
  int_col_s2n     = 34
  int_col_stn     = 35
  dblarr_rave_teff_raw = double(strarr_rave(*,int_col_teff))
  dblarr_rave_logg_raw = double(strarr_rave(*,int_col_logg))
  dblarr_rave_mh_raw = double(strarr_rave(*,int_col_mh))
  dblarr_rave_afe_raw = double(strarr_rave(*,int_col_afe))
  dblarr_rave_s2n = double(strarr_rave(*,int_col_s2n))
  dblarr_rave_stn = double(strarr_rave(*,int_col_stn))
  dblarr_rave_teff = dblarr_rave_teff_raw
  dblarr_rave_logg = dblarr_rave_logg_raw
  dblarr_rave_mh = dblarr_rave_mh_raw
  dblarr_rave_afe = dblarr_rave_afe_raw
  ...
  dblarr_rave_cmh_raw = dblarr_rave_cmh
...
;  stop
  ...
  str_htmlfile = str_path + 'index_'+strmid(str_filename,strpos(str_filename,'/',/REVERSE_SEARCH)+1)
  str_htmlfile = strmid(str_htmlfile,0,strpos(str_htmlfile,'.',/REVERSE_SEARCH))+'.html'
...
;                          I_DBLARR_LOGG                     = double(strarr_data(indarr(indarr_feh), int_col_logg_rave))
  ...
  endelse
  dblarr_teff_rave_raw = double(strarr_data(*,int_col_teff_rave))
  dblarr_logg_rave_raw = double(strarr_data(*,int_col_logg_rave))
  dblarr_mh_rave_raw   = double(strarr_data(*,int_col_mh_rave))
  dblarr_afe_rave_raw   = double(strarr_data(*,int_col_afe_rave))

;  dblarr_teff_rave_last = dblarr_teff_rave_raw
;  dblarr_logg_rave_last = dblarr_logg_rave_raw
;  dblarr_mh_rave_last = dblarr_mh_rave_raw
;  dblarr_afe_rave_last = dblarr_afe_rave_raw

  rave_calibrate_metallicities,I_DBLARR_MH            = dblarr_mh_rave_raw,$
                                I_DBLARR_AFE           = double(strarr_data(*,int_col_afe_rave)),$
                                I_DBLARR_TEFF          = dblarr_teff_rave_raw,$; --- new calibration
                                I_DBLARR_LOGG          = dblarr_logg_rave_raw,$; --- old calibration
                                I_DBLARR_STN           = double(strarr_data(*,int_col_stn_rave)),$; --- calibration from DR3 paper
                                O_STRARR_MH_CALIBRATED = strarr_mh_rave_calibrated_raw,$;           --- string array
                                I_DBL_REJECTVALUE      = 9.99,$; --- double
                                I_DBL_REJECTERR        = 1,$;       --- double
                                I_B_SEPARATE           = 1
  dblarr_cmh_rave_raw = double(strarr_mh_rave_calibrated_raw)
  ;dblarr_cmh_rave_last = dblarr_cmh_rave_raw

  dblarr_teff_calib = dblarr_teff_rave_raw
  dblarr_logg_calib = dblarr_logg_rave_raw
  dblarr_mh_calib = dblarr_mh_rave_raw
  dblarr_cmh_calib = dblarr_cmh_rave_raw
  dblarr_afe_calib = dblarr_afe_rave_raw

  ii_start = 0
  if b_do_calib then $
    ii_start = 1
  for ii=ii_start, 2 do begin; --- dwarfs and giants
;ii = 1
    if ii eq 0 then begin
      b_dwarfs_only = 0
      b_giants_only = 0
...
;    endif
    ...
    dblarr_position_quad=[0.166,0.24,0.92,0.995]
    ...
    print,'rave_compare_to_external_and_calibrate_teff: dblarr_teff_rave_raw = ',dblarr_teff_rave_raw
    print,'rave_compare_to_external_and_calibrate_teff: dblarr_teff_ext = ',strarr_data(*, int_col_teff_ext)
    print,'rave_compare_to_external_and_calibrate_teff: size(dblarr_teff_rave_raw) = ',size(dblarr_teff_rave_raw)
    ...
    if indarr_no_logg(0) ge 0 then $
      dblarr_logg_ext_rave(indarr_no_logg) = dblarr_logg_rave_raw(indarr_no_logg)
...
;    stop
    ...
    for iii = 0,int_niter do begin; --- number of complete iterations
...
;        end else begin
      ...
      if b_calib_with_afe_first_run_only and (iii eq 1) then $
        int_cal_end -= 1

      int_cal_end_temp = int_cal_end
      if iii eq int_niter then $
        int_cal_end_temp = 0

      for int_cal = 0, int_cal_end_temp do begin
        i_do_calib = 0
        if int_cal eq 0 then begin; --- STN
          dblarr_calib = double(strarr_data(indarr_logg, int_col_stn_rave))
          dblarr_calib_x = dblarr_calib
          dblarr_calib_all = dblarr_rave_stn(indarr_logg_all)
          dblarr_calib_all_x = dblarr_calib_all
          ...
          if b_calib_vs_raw_data then begin
            dblarr_calib_x = dblarr_teff_rave_raw(indarr_logg)
            dblarr_calib_all_x = dblarr_rave_teff_raw(indarr_logg_all)
          end else begin
            dblarr_calib_x = dblarr_calib
            dblarr_calib_all_x = dblarr_calib_all
          endelse
          ...
          str_xtickformat_calib = '(I6)'
          i_do_calib = 1
        end else if (int_cal eq 2) and (b_calib_mh or b_calib_mh_cmh) then begin; --- [m/H]
          dblarr_calib = dblarr_mh_calib(indarr_logg)
          dblarr_calib_all = dblarr_rave_mh(indarr_logg_all)
          if b_calib_vs_raw_data then begin
            dblarr_calib_x = dblarr_mh_rave_raw(indarr_logg)
            dblarr_calib_all_x = dblarr_rave_mh_raw(indarr_logg_all)
          end else begin
            dblarr_calib_x = dblarr_calib
            dblarr_calib_all_x = dblarr_calib_all
          endelse
          if b_dwarfs_only then begin
            ...
            dblarr_xrange_calib = [-2.5,0.5]
          endif
          str_xtitle_calib = '[m/H]!DRAVE!N [dex]'
          str_calib = 'mH'
          str_xtickformat_calib = '(F4.1)'
          i_do_calib = 1
        end else if ((int_cal eq 2) and b_calib_cmh) or ((int_cal eq 3) and b_calib_mh_cmh) then begin; --- [M/H]
          dblarr_calib = dblarr_cmh_calib(indarr_logg)
          dblarr_calib_all = dblarr_rave_cmh(indarr_logg_all)
          if b_calib_vs_raw_data then begin
            dblarr_calib_x = dblarr_cmh_rave_raw(indarr_logg)
            dblarr_calib_all_x = dblarr_rave_cmh_raw(indarr_logg_all)
          end else begin
            dblarr_calib_x = dblarr_calib
            dblarr_calib_all_x = dblarr_calib_all
          endelse
          ...
          str_xtickformat_calib = '(F4.1)'
          i_do_calib = 1
        end else if ((int_cal eq 3) and (b_calib_mh or b_calib_cmh)) or ((int_cal eq 4 and b_calib_mh_cmh)) then begin
          dblarr_calib = dblarr_logg_calib(indarr_logg)
          dblarr_calib_all = dblarr_rave_logg(indarr_logg_all)
          if b_calib_vs_raw_data then begin
            dblarr_calib_x = dblarr_logg_rave_raw(indarr_logg)
            dblarr_calib_all_x = dblarr_rave_logg_raw(indarr_logg_all)
          end else begin
            dblarr_calib_x = dblarr_calib
            dblarr_calib_all_x = dblarr_calib_all
          endelse
          dblarr_xrange_calib = [0.,5.5]
          if b_dwarfs_only then begin
            dblarr_xrange_calib = [3.,5.5]
          end else if b_giants_only then begin
            dblarr_xrange_calib = [0.,4.]
          endif
          str_xtitle_calib = '(log g)!DRAVE!N [dex]'
          str_calib = 'logg'
          str_xtickformat_calib = '(F4.1)'
          i_do_calib = 1
        end else if ((int_cal eq 4) and (b_calib_mh or b_calib_cmh)) or ((int_cal eq 5) and b_calib_mh_cmh) then begin
          dblarr_calib = dblarr_afe_calib(indarr_logg)
          dblarr_calib_all = dblarr_rave_afe(indarr_logg_all)
          if b_calib_vs_raw_data then begin
            dblarr_calib_x = dblarr_afe_rave_raw(indarr_logg)
            dblarr_calib_all_x = dblarr_rave_afe_raw(indarr_logg_all)
          end else begin
            dblarr_calib_x = dblarr_calib
            dblarr_calib_all_x = dblarr_calib_all
          endelse
        ...
        print,'int_cal = ',int_cal,': str_calib = ',str_calib
;        if int_cal eq 3 then stop
        if iii eq int_niter then $
          i_do_calib = 0
;        i_start = 60
;        i_end = 69
;        if int_cal eq int_cal_end then begin
          i_start = 0
        if b_do_calib and i_do_calib then begin
          i_end = 73
        end else begin
          i_end = 59
        endelse
;        endif
  ...
  ;          indarr_clipped = indarr_clipped_logg
            ;b_logg = 1
            dblarr_position_diff=dblarr_position_quad;[0.17,0.252,0.92,0.995]
            b_quadratic = 1
            ...
            ;dblarr_coeffs_dlogg_vs_dteff = svdfit(dblarr_x,dblarr_y,2,YFIT=dblarr_yfit)
  ;          indarr_clipped = indarr_clipped_logg
            dblarr_position_diff=dblarr_position_quad;[0.197,0.292,0.92,0.995]
            b_quadratic = 1
            b_dont_plot_colour_bar = 1
...
; --- calibrate
    ...
    ;          endif
              str_plotname_root = str_path + 'all_found_MH-ext-no-FeH_vs_MH-RAVE-giants'
              i_xticks = 0
              i_yticks = 0
      ...
      ;          dblarr_yrange_diff = [-1.,1.]
      ;        endif
              str_plotname_root = str_path + 'all_found_dMH_vs_STN-RAVE-calib'
              i_xticks = 0
              i_yticks = 0
              str_xtickformat = '(I6)'
              str_ytickformat = '(I6)'
      ...
      ;          dblarr_yrange_hist = [0.,21.]
      ;        endif
              str_plotname_root = str_path + 'all_found_MH-RAVE_vs_MH-ext-giants-from-FeH-Zwitter'
              i_xticks = 0
              i_yticks = 0
              str_xtickformat = '(F6.1)'
              str_ytickformat = '(F6.1)'
      ...
      ;        if b_giants_only then $
      ;          dblarr_yrange_hist = [0.,21.]
              str_plotname_root = str_path + 'all_found_MH-RAVE_vs_MH-ext-calib-stn'
              i_xticks = 0
              i_yticks = 0
              str_xtickformat = '(F6.1)'
              str_ytickformat = '(F6.1)'
    ...
    ;            dblarr_yrange_hist = [0.,21.]
    ;          endif
              str_plotname_root = str_path + 'all_found_MH-ext_vs_MH-RAVE-giants-from-FeH-mine'
              i_xticks = 0
              i_yticks = 0
              str_xtickformat = '(F6.1)'
              str_ytickformat = '(F6.1)'
...
;stop
          ...
          if indarr_clipped(0) ge 0 then begin
            ...
            if b_teff then begin
              ...
              endelse
              print,'rave_compare_to_external_and_calibrate: indarr_clipped_teff = ',indarr_clipped_teff
...
;            end else if int_do_calib eq 10 then begin
;              str_filename_calib = str_plotname_root+strtrim(string(long(int_sigma_nbins)),2)+'sigma-bins'
;            end else if int_do_calib eq 11 then begin
...
;            end else if int_do_calib eq 12 then begin
...
;            end else if int_do_calib eq 13 then begin
...
;            end else if int_do_calib eq 14 then begin
...
;            end else if int_do_calib eq 15 then begin
...
;            end else if int_do_calib eq 16 then begin
;              str_filename_calib = '/home/azuri/daten/rave/rave_data/release8/calib_mH-calib_vs_Teff-calib_calib-mH_calib-logg_'+strtrim(string(long(int_sigma_nbins)),2)+'sigma-bins'
;            end else if int_do_calib eq 17 then begin
;              str_filename_calib = '/home/azuri/daten/rave/rave_data/release8/calib_mH-calib_calib-Teff_vs_logg-calib_calib-mH_calib-logg_'+strtrim(string(long(int_sigma_nbins)),2)+'sigma-bins'
;            end else if int_do_calib eq 18 then begin
...
;            end else if int_do_calib eq 19 then begin
...
;            end else if int_do_calib eq 20 then begin
...
;            end else if int_do_calib eq 21 then begin
...
;            end else if int_do_calib eq 22 then begin
...
;            end else if int_do_calib eq 23 then begin
;              str_filename_calib = '/home/azuri/daten/rave/rave_data/release8/calib_logg-calib_calib-mH-calib-Teff_vs_afe-calib_calib-mH_calib-Teff-calib-logg_'+strtrim(string(long(int_sigma_nbins)),2)+'sigma-bins'
  ...
  ;          endif
            ...
            if (int_do_calib eq 1) then begin; or (int_do_calib eq 10) then begin
...
;              endif
    ...
    ;      endif
          if i eq 0 then begin
            ...
            str_ytitle_diff = '[M/H]!Dext!N - [m/H]!DRAVE!N [dex]'
            str_parameter = 'MH'
            dblarr_xrange_dwarfs = [-2.,0.5]
            dblarr_yrange_dwarfs = [-2.,0.5]
            dblarr_yrange_diff_dwarfs = [-1.,1.]
...
;  strarr_data(*,int_col_teff_rave) = strtrim(string(dblarr_teff_calib(i)),2)
;  strarr_data(*,int_col_teff_rave) = strtrim(string(dblarr_teff_calib(i)),2)
;  strarr_data(*,int_col_teff_rave) = strtrim(string(dblarr_teff_calib(i)),2)
;  strarr_data(*,int_col_teff_rave) = strtrim(string(dblarr_teff_calib(i)),2)
    ...
    if b_do_all then begin
      ...
      for i=0ul, n_elements(strarr_rave(*,0))-1 do begin
        ...
        for j=1,n_elements(strarr_rave(0,*))-1 do begin
          ...
          end else if j eq int_col_mh_out then begin
          ...
          end else if j eq int_col_cmh_out then begin
            str_line_out = str_line_out + ' ' + strtrim(string(dblarr_rave_cmh(i)),2)
          end else begin
            str_line_out = str_line_out + ' ' + strarr_rave(i,j)
          endelse
        endfor
        str_line_out = str_line_out + ' ' + strtrim(string(dblarr_rave_cmh_raw(i)),2)
        printf,lun,str_line_out
      endfor
      free_lun,lun
    endif
  ...
  if b_fit_gaussians then begin
    rave_mh_gaussfits, I_STR_FILENAME = str_filename_rave_out,$
                      I_STR_OUTFILENAME = strmid(str_filename_rave_out,0,strpos(str_filename_rave_out,'.',/REVERSE_SEARCH))+'_gaussfits_mH-calib',$
                      I_B_DWARFS     = 0,$
                      I_INT_COL_FIT  = int_col_mh_out
