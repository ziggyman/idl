...
PRO GAUSS_FUNCT,X,A,F,PDER
    ...
    ;print,'gauss_funct: x = ',x
    ;print,'gauss_funct: a = ',a
    n = n_elements(a)
    nx = N_ELEMENTS(x)
    ...
    if a[2] ne 0.0 then begin
        ...
        ez = z*0
    endelse
    ;print,'gauss_funct: z = ',z
    ;print,'gauss_funct: ez = ',ez
...
Function Gaussfit, x, y, a, $
    ...
    if (nEst eq 0 || est[2] eq 0) then begin  ;Compute estimates?
     ...
     ;   print,'gaussfit: yd = ',yd
        ...
        ymax=max(yd, imax)
     ;   print,'gaussfit: ymax = ',ymax
        xmax=x[imax]
     ;   print,'gaussfit: xmax = ',xmax
        ymin=min(yd, imin)
     ;   print,'gaussfit: ymin = ',ymin
        xmin=x[imin]
     ;   print,'gaussfit: xmin = ',xmin
        if abs(ymax) gt abs(ymin) then i0=imax else i0=imin ;emiss or absorp?
     ;   print,'gaussfit: i0 = ',i0
...
;    print,'gaussfit: weights = ',weights
;    print,'gaussfit: a = ',a
;    print,'gaussfit: sigma = ',sigma
;    print,'gaussfit: chisq = ',chisq
;    print,'gaussfit: yerror = ',yerror
