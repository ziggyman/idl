...
;############################
pro calcsnr_compare_list,filelist,arealist
;############################
;
; NAME:                  calcsnr_compare_list
...
; CALLING SEQUENCE:      calcsnr_compare_list,'HD_textfiles.list','testareas.list'
...
if n_elements(arealist) eq 0 then begin
    print,'calcsnr_compare_list: Not enough parameters, return 0.'
    print,"calcsnr_compare_list: USAGE: calcsnr_compare_list,'HD_textfiles.list','testareas.list'"
end else begin
...
;countlines
  ...
  print,'calcsnr_compare_list: ',filelist,': ',maxn,' FILES'

  nareas = 0UL
  nareas = countlines(arealist)
  print,'calcsnr_compare_list: ',arealist,': ',nareas,' FILES'
...
;read file in arrays
  iq = ' '
  print,'calcsnr_compare_list: reading file ',filelist
  close,1
  openr,1,filelist
  for i=0,maxn-1 do begin
    readf,1,iq
    file(i) = path+strtrim(iq,2)
  end
  close,1  

  print,'calcsnr_compare_list: reading file ',arealist
  close,1
  openr,1,arealist
  ...
  for i=0,maxn-1 do begin
      ...
      datfilename = strmid(file(i),0,strpos(file(i),'.',/REVERSE_SEARCH))+'.data'
