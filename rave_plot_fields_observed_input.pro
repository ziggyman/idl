pro rave_plot_fields_observed_input,DBLARR_RAVEDATA=dblarr_ravedata,$    ; --- dblarr(*,3)
...
;
; NAME:                  rave_plot_fields_observed_input.pro
; PURPOSE:               plots the fields in the plot DEC over RA
; CATEGORY:              RAVE
; CALLING SEQUENCE:      rave_plot_fields_observed_input,dblarr_fields,strarr_meansigfiles,str_plotname
...
;
  ...
  ; --- rave
    ...
    print,'rave_plot_fields_observed_input: rave: n_elements(indarr_lon) = ',n_elements(indarr_lon)

    if n_elements(indarr_lon) eq 1 and indarr_lon(0) eq -1 then begin
      lonarr_all_stars_rave(i) = 0
      lonarr_bad_stars(i) = 0
      print,'rave_plot_fields_observed_input: rave: n_elements(indarr_lon) eq 1 and indarr_lon(0) eq -1'
    end else begin
      ...
      print,'rave_plot_fields_observed_input: rave: n_elements(indarr_lat) = ',n_elements(indarr_lat)
      if n_elements(indarr_lat) eq 1 and indarr_lat(0) eq -1 then begin
        lonarr_all_stars_rave(i) = 0
        lonarr_bad_stars(i) = 0
        print,'rave_plot_fields_observed_input: rave: n_elements(indarr_lat) eq 1 and indarr_lat(0) eq -1'
      end else begin
        print,'rave_plot_fields_observed_input: dblarr_ravedata_plot(indarr_lon(indarr_lat),i_dat) = ',dblarr_ravedata_plot(indarr_lon(indarr_lat),i_dat)
...
;        print,'rave_plot_fields_observed_input: dblarr_ravedata_plot(indarr_lon(indarr_lat),i_dat) = ',dblarr_ravedata_plot(indarr_lon(indarr_lat),i_dat)
        ...
        if n_elements(indarr_bad_stars) eq 1 and indarr_bad_stars(0) eq -1 then begin
          lonarr_bad_stars(i) = 0
          print,'rave_plot_fields_observed_input: rave: n_elements(indarr_bad_stars) eq 1 and indarr_bad_stars(0) eq -1'
;          if i_dat eq 6 and i eq i_nfields-1 then $
;            stop
        end else begin
          print,'rave_plot_fields_observed_input: dblarr_ravedata_plot(indarr_lon(indarr_lat(indarr_bad_stars)),i_dat) = ',dblarr_ravedata_plot(indarr_lon(indarr_lat(indarr_bad_stars)),i_dat)
          lonarr_bad_stars(i) = n_elements(indarr_bad_stars)
          print,'rave_plot_fields_observed_input: rave: lonarr_bad_stars(',i,') = ',lonarr_bad_stars(i)
;          print,'rave_plot_fields_observed_input: rave: dblarr_ravedata_plot(indarr_lon(indarr_lat(indarr_bad_stars)),i_dat) = ',dblarr_ravedata_plot(indarr_lon(indarr_lat(indarr_bad_stars)),i_dat)
        end
      end
    end
...
; --- besancon
    ...
    print,'rave_plot_fields_observed_input: besancon: n_elements(indarr_lon) = ',n_elements(indarr_lon)
    ...
    if n_elements(indarr_lon) eq 1 and indarr_lon(0) eq -1 then begin
      ...
      print,'rave_plot_fields_observed_input: besancon: n_elements(indarr_lat) = ',n_elements(indarr_lat)
      if n_elements(indarr_lat) eq 1 and indarr_lat(0) eq -1 then begin
        lonarr_all_stars_besancon(i) = 0
        lonarr_bad_stars(i) = 0
        print,'rave_plot_fields_observed_input: besancon: n_elements(indarr_lat) eq 1 and indarr_lat(0) eq -1'
      end else begin
        lonarr_all_stars_besancon(i) = n_elements(indarr_lat)
        print,'rave_plot_fields_observed_input: besancon: lonarr_all_stars_besancon(',i,') set to ',lonarr_all_stars_besancon(i)
      end
    end
  end
  print,'lonarr_all_stars_besancon = ',lonarr_all_stars_besancon
  print,'lonarr_all_stars_rave = ',lonarr_all_stars_rave
  stop
...
; --- observed stars vs. besancon stars
  print,'rave_plot_fields_observed_input: creating str_plotname='+str_plotname
  set_plot,'ps'
  ...
  for i=0UL, i_nfields-1 do begin
    ...
    if lonarr_all_stars_rave(i) lt 1 then begin
      ...
      print,'rave_plot_fields_observed_input: lonarr_all_stars_rave(',i,') = ',lonarr_all_stars_rave(i),', lonarr_all_stars_besancon(',i,') = ',lonarr_all_stars_besancon(i)
      dbl_percentage = (double(lonarr_all_stars_rave(i)) / double(lonarr_all_stars_besancon(i))) * 100.
      print,'rave_plot_fields_observed_input: dbl_percentage = ',dbl_percentage
      colorx = long((dbl_percentage / dbl_maxpercentage) * 254.)
      if colorx lt 0 then colorx = 0
      if colorx gt 254 then colorx = 254
      print,'rave_plot_fields_observed_input: colorx = ',colorx
    end
;    print,'rave_plot_fields_observed_input: starting to plot field: colorx = ',colorx,', b_plot = ',b_plot
...
; --- observed stars with parameters vs. besancon stars
  ...
  for i=0UL, i_nfields-1 do begin
    ...
    if lonarr_all_stars_rave(i) lt 1 then begin
      ...
      print,'rave_plot_fields_observed_input: dblarr_fields(',i,',*) = ',dblarr_fields(i,*)
