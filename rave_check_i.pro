...
;    print,'intarr_hist(i=',i,') = ',intarr_hist(i)
  ...
  if i_n_digits eq 1 then begin
    ...
    str_xtitle = 'RAVE I magnitude last digit'
  end else begin
    str_filename = str_filename+'both_digits_behind_dot.ps'
    str_xtitle = 'RAVE I magnitude last two digits'
  end
  device,filename=str_filename

  plot,[-.5,(10.^i_n_digits)- .5],[0.,0.],xrange=[-.5,(10.^i_n_digits)- .5],yrange=[0.,double(max(intarr_hist))+(double(max(intarr_hist))/20.)],xstyle=1,ystyle=1,xtitle=str_xtitle,ytitle='Number of stars',charsize=2,charthick=2
  for i=0ul, (10^i_n_digits)-1 do begin
