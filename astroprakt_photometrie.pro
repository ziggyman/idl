...
;############################
pro astroprakt_photometrie,filelist
;############################
...
if n_elements(filelist) eq 0 then print,'Beispiel: No file specified, return 0.' $
  ...
  for i=0,maxn-1 do begin  
    readf,1,fileq  
    files(i)=fileq
    if strmid(fileq,0,0) eq 'b' then nbias = nbias + 1
    if strmid(fileq,1,1) eq 'f' then nflat = nflat + 1
  end  
  close,1  
  ...
  for i=0,maxfiles-1 do begin
    if strmid(files(i),0,0) eq 'b' then begin
      bias(ibias)=files(i)
      ibias = ibias + 1
    endif else begin
      if strmid(files(i),1,1) eq 'f' then begin
        flat(iflat)=files(i)
        iflat = iflat + 1
      endif
    endelse
  ...
  for i=1,nbias-1 do combinedbias = combinedbias + readfits(bias(i), h)
  ...
  for i=1,nflat-1 do combinedflat = combinedflat + readfits(flat(i), h)
