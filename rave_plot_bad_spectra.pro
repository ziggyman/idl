pro rave_plot_bad_spectra,STR_PATH=str_path,STR_FILELIST=str_filelist,STR_TEXFILE=str_texfile
;
; NAME:                  rave_plot_bad_spectra.pro
...
;
  ...
  if not keyword_set(STR_FILELST) then $
  ...
  for i=0UL, i_n_files-1, i_n_plots do begin
    ...
    device,filename=str_psfilename,xsize=18,ysize=28,/color,encaps=1
      for j=0UL,min([i_n_plots,i_n_files-i])-1 do begin
        ...
        strarr_data = readfiletostrarr(str_path+strarr_filelist(i+j),' ')
        ...
        str_title = strmid(strarr_filelist(i+j),strpos(strarr_filelist(i+j),'/')+1)
