...
;############################
pro skyview,list,texfile,path,runid
;############################
...
if n_elements(runid) eq 0 then begin
  ...
  print,'usage: skyview,starlist,texfile,path_where_the_images_are,runid'
endif else begin   
...
;        printf,1,'skyview: ---------- strmid(mytexarr('+string(j)+'),0,'+string(strlen(tempstring3))+') = '+strmid(mytexarr(j),0,strlen(tempstring3))
          endif else if strmid(mytexarr(j),0,strlen(tempstring3)) eq tempstring3 then begin
...
;        endif else printf,filenr,mytexarr(j)
        endif else if strmid(mytexarr(j),0,strlen(tempstring5)) eq tempstring5 then begin
          printf,1,'skyview: Treffer! replacing one RUN ID'
          print,'skyview: Treffer! replacing one RUN ID'
          printf,filenr,tempstring5+runid+tempstring6
